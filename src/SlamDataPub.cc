...
void SlamDataPub::TrackingDataPub()
{
    ...
    while(1)
    {
        if(mbGetNewCamPose)
        {
            ...
            float tf_q2_x = camPose2Ground.pose.orientation.x;
            float tf_q2_y = camPose2Ground.pose.orientation.y;
            float tf_q2_z = camPose2Ground.pose.orientation.z;
            float tf_q2_w = camPose2Ground.pose.orientation.w;
            ...
            Camera2Ground_broadcaster_.sendTransform(
                tf::StampedTransform(
                    tf::Transform(tf::Quaternion(tf_q2_x,tf_q2_y,tf_q2_z,tf_q2_w), tf::Vector3(tf_x, tf_y, tf_z)),
                    ros::Time::now(),"ground", "camera"));
